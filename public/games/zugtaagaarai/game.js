...
class Game {
  constructor() {
    ...
    this.gameOver = false;
    this.score = 0;
    this.highScore = localStorage.getItem("Өндөр оноо") || 0;
  ...
  drawUI() {
    ...
    ctx.font = "20px Arial";
    ctx.textAlign = "зүүн";
    ctx.fillText(`Оноо: ${this.score}`, 10, 25);
    ctx.textAlign = "зөв";
    ctx.fillText(`Өндөр оноо: ${this.highScore}`, canvas.width - 10, 25);

    ctx.textAlign = "center";
    if (this.scoreMultiplier > 1) {
      ctx.fillStyle = "#ffd700";
      ctx.fillText(
        `Үржүүлэгч: x${this.scoreMultiplier}`,
        canvas.width / 2,
        25
      );
    }
    if (this.shieldActive) {
      ...
      ctx.fillText("Бамбай идэвхтэй!", canvas.width / 2, 50);
    }
    if (this.timeSlowed) {
      ctx.fillStyle = "#8a2be2";
      ctx.fillText("Цаг удааширсан!", canvas.width / 2, 75);
    }
  }
  ...
  endGame() {
    ...
    ctx.font = "40px Arial";
    ctx.textAlign = "center";
    ctx.fillText("Тоглоом дууслаа", canvas.width / 2, canvas.height / 2 - 40);
    ctx.font = "24px Arial";
    ctx.fillText(
      `Таны оноо: ${this.score}`,
      canvas.width / 2,
      canvas.height / 2
    );
    ctx.fillText(
      "Дахин эхлүүлэхийн тулд SPACE дарна уу",
      canvas.width / 2,
      canvas.height / 2 + 40
    );
  }
  ...
  updateHighScore() {
    if (this.score > this.highScore) {
      ...
      localStorage.setItem("Өндөр оноо", this.highScore);
    }
  }
